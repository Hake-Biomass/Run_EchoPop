...
DATA_ROOT = Path("C:/rthomas/Projects/EchoPop_validation/Data/From UW/echopop2023_file_copy")
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# NASC EXPORTS FILE(S)
NASC_EXPORTS_FILES = DATA_ROOT / "raw_nasc/"  #directory that contains Echoview outputs - contains analysis, cells, etc
# NASC EXPORTS SHEET
...
if NASC_PREPROCESSED:
    ...
    df_intervals, df_exports = ingestion.nasc.merge_echoview_nasc(
        ...
        default_latitude_threshold = 60.0,  #anything north of this, toss out
    )
#df_intervals - lat, long, etc, df_exports has region id, region name, region classes, etc.
    # EXPORT REGION NAME MAPPING
    REGION_NAME_EXPR_DICT = {  
        "REGION_CLASS": {  #look for these region classes
    ...
    df_exports_with_regions = ingestion.nasc.process_region_names(
        ...
        can_haul_offset=200,
    )  #If don't generate but read in, is in a different year workflow
    ...
    df_nasc = ingestion.nasc.consolidate_echvoiew_nasc(
        ...
        region_class_names=["Age-1 Hake", "Age-1", "Age-1 Hake Mix", "Hake", "Hake Mix"],  #dont worry about taking this out for age-2+ only, but can.
        impute_region_ids=True, #makes all regino numbers the same
...
# .  This was Chu's way of avoiding double-counting - sometimes double-count if no fish in length.  May end up changing this? ...Total specimen weight treated separately and then combined....btw..catch file is total weight of everything - includes specimen weights
# ==================================================================================================
...
FEAT_TO_ECHOPOP_STRATA_COLUMNS = {  #can change from year toyear
    "wt": "nasc_proportion",
    "haul": "haul_num",
    "inpfc": "stratum_num",
...
df_dict_strata = ingestion.load_strata(  #reads in strata file
...
df_dict_geostrata = ingestion.load_geostrata(  #load in the geostrata
...
df_isobath = ingestion.load_isobath_data( #From Chu -Chu's file didnt have column names, so slightly modified.  Is 200 m isobath file
...
FEAT_TO_ECHOPOP_GEOSTATS_PARAMS_COLUMNS = {#kmax and kmin come in as well, but in naming expected by echopop
...
# ---- BIODATA [INPFC] - adds column with stratum to biodata
...
# ---- NASC [INPFC] - adds column with stratum to NASC
...
# ---- MESH [INPFC]
df_mesh = ingestion.join_geostrata_by_latitude(data=df_mesh, #add geostratum to mesh data
                                               geostrata_df=df_dict_geostrata["inpfc"], 
                                               stratum_name="geostratum_inpfc")
# ---- MESH [KS]
...
# AGE-BINS - make the bins.  Assumes evenly spaced bins.  Uneven spaced bins breaks things
AGE_BINS = np.linspace(start=1., stop=22, num=22)#EchoPop default of 20 causes issues because 21 and 22 gets lumped into age20.  This may not actually be an issue for us (Did Chu lump?)  Probably doesn't make a big difference either way
utils.binify(
...
# ALL FISH.  A bit different from how EchoPRo did - had assumed all unsexed fish were female.  Is now male, female, and unsexed
...
# SEX-SPECIFIC - weight at length by sex.  Not done by strata - length weight key is for whole survey
...
# ALL FISH - weight at length for each sex and all. Not done by strata - length weight key is for whole survey
...
MODEL_PARAMETERS = {
    ...
    "impute_missing_strata": True,  #eg. in inpfc missing biological data from that inpfc - how does one resolve that
    "haul_replicates": True,  #treat each haul as a single sample when calculating sigma_BS
}
...
# INVERT NUMBER DENSITY - NASC->number conversion using TS
...
transect.compute_interval_distance(df_nasc=df_nasc, interval_threshold=0.05)  #corrects intervals where median distance is more tha 0.05 from median distance (Sets to median distance?) 
...
# COMPUTE ABUNDANCE (from number density, and add in by sex)
logging.info(
...
# COMPUTE STRATUM-AVERAGED WEIGHTS.  Ave weight of a single fish in each stratum. Except that tis all sums up the M and F.  For each of M and F, the weight that is in there isn't really the weight of one fish - but kind of the proportional weight 
...
# BIOMASS [AGED-ONLY]- by length, age, sex
...
vgm = geostatistics.Variogram(
    lag_resolution=0.002,  #in units of normalized coordinates
    n_lags=30,
    coordinate_names=("x", "y"),
)
logging.info("Variogram-class object 'vgm' created...")

# EMPIRICAL VARIOGRAM - if want to do own fitting
logging.info(
...
# SET UP FITTING PARAMETERS - default EchoPRo variogram was joint exponential bessel variogram
...
VARIOGRAM_PARAMETERS = {
    "model": ["exponential", "bessel"],
    **best_fit_parameters  #use **dict_Variogram_params if want to use variogram paramters that brought in
}
...
df_kriged_results = krg.krige(
    transects=df_nasc,
    variable="biomass_density",
    extrapolate=True,  #this doesn't automatically do mesh cropping if set to false.  Will error out if don't have a cropped method first.
...
# CONVERT TO BIOMASS (from biomass density)
...
# BIOMASS TO NASC (calculated by stratum) - use average weights.  When go from biomass to respective NASC and abundance, use average weight for all fish, not sex-specific.  USe stratified mean sigma bs falues calculated earlier
feat_apportion.mesh_biomass_to_nasc(
    ...
    mesh_biodata_link={"geostratum_ks": "stratum_ks"},  #what biodata get used
...
# ABUNDANCE [ALL] #when go from biomass to abundance - use average weights
logging.info("Distributing abundances...")
...
#redistribution of unaged values distributed over aged, and interpolate sex onto them.
...
# SCALE UNAGED BIOMASS to age and sex
logging.info(
...
#In aged and unaged data sets, may have length values unaged that are not in aged.  If mismatch in presence of 
#length bin.  For EchoPRo, takes closest fish of length, for EchoPop, takes closest fish of the same sex
#So now have two cubes of aged and unaged fish by sex, length, and age
# CONSOLIDATE these two tables
# ---- ABUNDANCE
...
JOLLYHAMPTON_PARAMETERS = {
    "transects_per_latitude": 5,  #this is for doing the kriged version of JH
    "strata_transect_proportion": 0.75,  #within each stratum
    "num_replicates": 1000,
}
...
jh.stratified_bootstrap(data_df=df_nasc, 
                        stratify_by=["geostratum_inpfc"], #use INPFC for the strata
                        variable="biomass")
logging.info(
    "Summarizing results....\n"
...
#unkriged results
# #bias column in transect results- may not get the same bootstrapped mean.  That bias estimate is the 
# the difference between the bootstrapped mean and the unkriged estimate.  Can also be done for each 
# stratum.  JH is the CV that comes out of it. 
logging.info("Stratified transect analysis results complete\n'df_jh_transect_results' created.")
#can set percentile or use some of the other built in CI methods
# RUN ON KRIGED DATA
...
kriged_transects = jh.create_virtual_transects(#for kriged, makes these 5 virtual transects per latitue
    data_df=df_kriged_results,
    geostrata_df=df_dict_geostrata["inpfc"], 
    stratify_by=["geostratum_inpfc"],
    variable="biomass",
) #makes virtual transects
# ---- Run rest of flow
logging.info(
...
#bias in the difference from the bootstrap output to the regular kriged estimate.  Positive results 
# boostrapped estimate is that much higher than the original estimator
